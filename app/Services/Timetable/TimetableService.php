...
class TimetableService
{
    ...
    private function validateBusinessRules(array $data, ?int $ignoreId = null): void
    {
        ...
        if ($overlap(
            ...
            throw ValidationException::withMessages([
                'teacher_assignment_id' => [
                    'This class already has another timetable during the selected time.',
                ],
            ]);
        }
    }
    ...
    // Teacher role - restrict general timetable query
    public function applyAccessScope(Builder $query, User $user): Builder
    {
        ...
        if ($role === 'TEACHER') {
            return $query->whereHas('teacherAssignment', function (Builder $q) use ($user) {
                $q->where('status', 'ACTIVE')
                    ->whereHas('teacher',
                        fn(Builder $teacherQuery) => $teacherQuery->where('user_id', $user->id));
            });
        }
    ...
    // Teacher role - compact own timetable
    public function getForTeacher(User $user)
    {
        return Timetable::query()
            ->whereHas('teacherAssignment', function (Builder $q) use ($user) {
                $q->where('status', 'ACTIVE')
                    ->whereHas('teacher',
                        fn(Builder $teacherQuery) => $teacherQuery->where('user_id', $user->id));
            })
            ->with([
                'teacherAssignment.schoolClass.grade',
                'teacherAssignment.subject',
                'room',
            ])
            ->orderByRaw("CASE day_of_week
